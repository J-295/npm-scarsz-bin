...
/** Extracts the UUID and key from a URL then calls `get`. */
...
export async function get(uuid: string, key: string): Promise<Bin | null> {
    ...
    if (typeof window !== "undefined") url = `https://corsproxy.io/?url=${encodeURIComponent(url)}`;
